...
import { readWitness, witnessVerdict } from '../web/btc-index.js';
...
test('unconfirmed records are not the chain saying anything', () => {
  ...
  assert.deepEqual(readWitness([pending], ADDR), { found: false, whole: true });
...
test('the four verdicts stay apart, and silence is never assent', () => {
  assert.equal(witnessVerdict(SPK, { found: true, script: SPK }), 'agrees');
  assert.equal(witnessVerdict(SPK.toUpperCase(), { found: true, script: SPK }), 'agrees', 'case is not a difference');
  assert.equal(witnessVerdict(SPK, { found: true, script: '0014' + 'ab'.repeat(20) }), 'differs');
