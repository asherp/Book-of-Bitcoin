...
// (the standard BIP39 English list, 2048 words = 11 bits) and HP_SPELLS is
// latin/payload_hp.yaml (64 words = 6 bits). Word INDEX is load-bearing --
// both lists are append-only upstream, so indices never shift -- and the two
// lists are disjoint, so a word identifies its own list. Kept as a static
// module (rather than fetched from the WASM bundle) so the version notation
// works synchronously and before/without WASM init.
...
export const BIP39 = (
  ...
  'broccoli broken bronze broom brother brown brush bubble buddy budget buffalo ' +
  'build bulb bulk bullet bundle bunker burden burger burst bus business busy ' +
  ...
  'firm first fiscal fish fit fitness fix flag flame flash flat flavor flee ' +
  ...
  'gauge gaze general genius genre gentle genuine gesture ghost giant gift ' +
  ...
  'load loan lobster local lock logic lonely long loop lottery loud lounge love ' +
  ...
  'own owner oxygen oyster ozone pact paddle page pair palace palm panda panel ' +
  ...
  'rude rug rule run runway rural sad saddle sadness safe sail salad salmon ' +
  ...
  'tray treat tree trend trial tribe trick trigger trim trip trophy trouble ' +
  'truck true truly trumpet trust truth try tube tuition tumble tuna tunnel ' +
  'turkey turn turtle twelve twenty twice twin twist two type typical ugly ' +
  ...
  'vintage violin virtual virus visa visit visual vital vivid vocal voice void ' +
  ...
  'work world worry worth wrap wreck wrestle wrist write wrong yard year yellow ' +
  'you young youth zebra zero zone zoo'
).split(' ');

export const HP_SPELLS = (
  'accio aguamenti alohomora avada crucio depulso descendo diffindo engorgio ' +
  'evanesca evanesco expecto expelliarmus finite flagrate homenum immobulus ' +
  'imperio incantatem incendio inflamari kedavra lacarnum legilimens levicorpus ' +
  'leviosa liberacorpus lumos morsmordre nox obliviate patronum petrificus ' +
  'protego reducto reparo revelio rictusempra sanentur sectumsempra ' +
  'serpensortia silencio sonorus stupefy totalus vera verto vipera vulnera ' +
  'wingardium anapneo aparecium ascendio bombarda colloportus deletrius ' +
  'densaugeo diminuendo flipendo furnunculus geminio glisseo impedimenta ' +
  'muffliato'
).split(' ');
