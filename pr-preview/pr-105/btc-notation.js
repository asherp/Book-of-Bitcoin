...
export const NOTATION_HTML = `
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid forms">
              <div class="glyph-row" data-marks="tpl:p2pk"><span class="g">P2PK</span><span class="m">Names a key outright. What opens it is a signature by that key, and nothing else is asked. <span class="why">The oldest of these locks and long since rare: the key stands in the open from the day it is written, rather than from the day it is used.</span></span></div>
              <div class="glyph-row" data-marks="tpl:p2pkh"><span class="g">P2PKH</span><span class="m">Names the <b>hash</b> of a key, so the key itself is nowhere in the passage. What opens it writes the key and a signature; the lock checks that the key hashes to <b>h</b>, then that the signature is that key’s. <span class="why">The form behind every address beginning 1.</span></span></div>
              <div class="glyph-row" data-marks="tpl:multisig"><span class="g">Multisig</span><span class="m">Names several keys and how many of them must sign — two of three, and so on. The one lock in the table with more than one blank. <span class="why">Written bare like this it takes no address, so it is nearly always found folded inside a P2SH and shows itself only when it is opened.</span></span></div>
              <div class="glyph-row" data-marks="tpl:p2sh tpl:p2sh-multisig"><span class="g">P2SH</span><span class="m">Names a hash and nothing else — not a key, not a condition, nothing whatever about what would satisfy it. So what opens it comes in two parts: first the marks that hash to <b>h</b>, which are themselves a lock, and then whatever <i>that</i> lock asks for. <span class="why">A condition of any size fits in twenty bytes this way, and none of it is written down until the day it is opened. The form behind every address beginning 3.</span></span></div>
            ...
            <div class="glyph-grid">
              <div class="glyph-row" data-marks="tpl:p2pk tpl:p2pkh tpl:multisig tpl:p2sh tpl:p2sh-multisig tpl:p2wpkh tpl:p2wsh tpl:p2tr-key tpl:p2tr-script tpl:data tpl:lightning"><span class="g">λ</span><span class="m"><b>the blank</b> — the part of a lock nobody has filled in. The letter after it is the one thing that differs between two locks of the same shape. <span class="why">Everything else on the line is marks the passage really carries, which is why ⧉ and its like stay where they stand rather than being tidied away. A blank still open is a thing not yet decided, and opening the lock is what decides it</span></span></div>
              <div class="glyph-row" data-marks="tpl:p2sh tpl:p2sh-multisig tpl:p2wsh tpl:p2tr-script"><span class="g">…</span><span class="m"><b>however many more</b> — a lock that hides a lock (P2SH, P2WSH, a tapscript leaf) hands back one wanting blanks of its own, and how many is a fact about marks this passage keeps behind a hash. <span class="why">So the notation says they are there and declines to count them. It leads because that is where they are written: whatever opens the lock writes them first, and the revealed lock stands on top. Every blank it can name is a letter; this one it cannot name</span></span></div>
            </div>
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row" data-marks="tpl:lightning"><span class="g"><b>p</b><sup>0</sup> <b>p</b><sup><i>i</i></sup> <b>p</b><sup>f</sup></span><span class="m"><b>channel state</b> — a superscript scopes a datum to a state of the channel: 0 the initial commitment sealed at funding, <i>i</i> any intermediate update, f the final, closing state. <span class="why">Each state the closing party draws a fresh secret k<sup><i>i</i></sup> and shares only its point p<sup><i>i</i></sup> = k<sup><i>i</i></sup>G; scripts and hashes rebuild per state. Advancing to <i>i</i>+1 revokes state <i>i</i>: k<sup><i>i</i></sup> crosses to the other side — and nothing else</span></span></div>
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              <div class="glyph-row"><span class="g">■<i>n</i></span><span class="m">the <b>block height</b>, written into the coinbase by the miner (BIP34). Here alone <i>n</i> is a raw height and not a count of chapters — the number the miner put in the bytes, in the chain's units rather than the book's. <span class="why">A block stating its own place is what makes every coinbase distinct. It is also the last thing any rule constrains — everything after it is the miner's own margin, arbitrary bytes in no format, so the book stops reading opcodes at this mark and renders the rest as what it is: quoted where the miner wrote words, prose where the bytes are only bytes. Printable is not the test — better than a third of a counter's bytes fall in the printable range by chance, and a quotation is the book saying someone wrote this</span></span></div>
              <div class="glyph-row" data-marks="β* re:256[⁰¹²³⁴⁵⁶⁷⁸⁹]"><span class="g">β<i>n</i> <i>m</i>×256<sup><i>e</i></sup></span><span class="m">the <b>difficulty target</b> the miner restated in the coinbase, in the chapter head's own notation — a valid hash opens with n zero bits, and the target is exactly the product beside the mark. <span class="why">The preamble is the older custom: before BIP34 a coinbase opened with this pair instead of a height</span></span></div>
              <div class="glyph-row" data-marks="time:template"><span class="g"><i>date</i></span><span class="m">the <b>template timestamp</b> — the moment the pool assembled the block template, written straight behind the height by the pools built on btccom's server. <span class="why">Printed in the chapter head's own date form because it is the same kind of thing the header states: a miner's clock reading. No rule puts it there and nothing in the bytes declares it; it is read as a clock because it agrees with the height standing beside it, and as a number otherwise</span></span></div>
              <div class="glyph-row" data-marks="η*"><span class="g">η<i>p</i>·<i>q</i>…</span><span class="m"><b>extranonce</b> — the counter a miner rolls once the header's 32 bits of η are exhausted. Read little-endian and minimally encoded, so the figure restores its own bytes, and written at full size as the number it is, factored. <span class="why">Where a pool leaves the gap its miners fill, this is what lands in it: behind ■ from BIP34 on, behind β before it, behind the template's date where one was written. The value rode small beside the glyph while it was one figure, but a factorization lowered into a subscript is a product nobody could read — and full size keeps it out of the margin below, where a counter's bytes are entropy and better than a third of them pass for printable text by chance. Most pools do not push it at all — F2Pool, AntPool and SECPOOL write the counter as raw bytes in the margin — and it takes the same mark there, since it is the same field. A zero byte closing such a counter is not part of the figure and comes off under ⓪, above, where it belongs. Past eight bytes a run in the margin is not a counter but a commitment or a datum, and stays prose rather than a hundred-digit figure that says nothing about what it holds</span></span></div>
              <div class="glyph-row" data-marks="zero:run"><span class="g">⓪<sup><i>n</i></sup></span><span class="m"><b><i>n</i> zero bytes</b> — ⓪ is the zero opcode (0x00, the byte itself) and the superscript counts bytes, as every superscript in a script does. A reading of length, not of meaning. <span class="why">In a coinbase it is the space a pool's template laid out and nothing filled: room for the counter a miner rolls, or for a commitment this block is not carrying. In a witness or a script's own push it is whatever left the room empty there — a Taproot internal key chosen as a nothing-up-my-sleeve value, the padding inside an inscription's payload — and the bytes state only that they are zero, which is why the hover says so and no more. The count restores the bytes exactly, and the alternative is a paragraph of the wordlist's first word, which is what zero encodes to. The chapter head's ⓪<i>n</i> counts zero <i>bits</i> of a block hash — same glyph, and each register measures in the unit its own thing is measured in</span></span></div>
              <div class="glyph-row" data-marks="sig:pool"><span class="g">“<i>tag</i>”</span><span class="m">a <b>pool's signature</b> — the name a mining pool wrote into its own margin, quoted to exactly the extent the pool wrote it and no further, so the counter byte that leans against a tag is not mistaken for the pool's punctuation. <span class="why">The extent comes from the book's table of signatures (<i>web/btc-pools.js</i>). Two claims, kept apart: that these bytes are in the coinbase is the record, and hovering names the pool the book reads them as — an inference from an unauthenticated string anyone may copy, which is why the name is not set on the page</span></span></div>
              <div class="glyph-row"><span class="g">⋔<sub>w</sub></span><span class="m">the <b>witness commitment</b> — in the coinbase, an OP_RETURN carrying ⌘(witness-tree root ‖ reserved value): the block's witnesses — its footnotes — bound to the chain through §1. <span class="why">The root is committed, never written; no wtxid appears on chain, only this testimony to all of them</span></span></div>
            </div>
          </div>
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row"><span class="g">⓪<sup><i>n</i></sup></span><span class="m">a block hash's <i>n</i> <b>proof-of-work zero bits</b>, closing the line after the prose. Here <i>n</i> counts bits, where the coinbase margin's ⓪<i>n</i> above counts bytes. <span class="why">They lead the hash as it is displayed, but the prose encodes internal order, where they trail: decode it, append them, reverse. The mark sits where the bytes do. A hash's leading zeros are what the work bought; bytes are what a pool leaves empty</span></span></div>
              ...
              <div class="glyph-row" data-marks="re:256[⁰¹²³⁴⁵⁶⁷⁸⁹]"><span class="g"><i>m</i>×256<sup><i>e</i></sup></span><span class="m">the <b>difficulty target</b> written <b>exactly</b>: its <b>mantissa</b>, times the whole-byte <b>shift</b> it rides on — the two parts nBits is made of. The whole factorization is in the hover. <span class="why">256<sup><i>e</i></sup> is the target's scale, said in the unit the header keeps it in — a retarget moves it by whole bytes — and <i>m</i> is the twenty-odd bits a window's work is actually chosen from. The mantissa is written as short as it goes: factored, then each part taken as its power or its figure, whichever is fewer characters, and neighbours multiplied back together wherever that is shorter still. Most mantissas come out as the plain figure, which is the honest answer — at six digits a factorization was buying the reader nothing — and a power that earns its place survives (2²²·3). In the hover, genesis' 65535 = 2¹⁶−1 opens as the Fermat primes 3·5·17·257. Its leading zeros are β's demand. The early coinbase preamble carries the same pair</span></span></div>
              <div class="glyph-row" data-marks="η*"><span class="g">η<i>p</i>·<i>q</i>…</span><span class="m">the <b>nonce</b> the miner landed on, <b>factored</b> — and factored in full, unlike the target above it, which is shortened to whatever writes in fewest characters. <span class="why">The difference is the reading. A target is chosen: a small mantissa on a scale of whole bytes, and the figure says that as well as the primes do. A nonce is arrived at by counting, and its two or three arbitrary primes — one of them usually enormous — are the whole of what there is to say about it. Shortening that to a figure would take away the only thing it shows</span></span></div>
              <div class="glyph-row"><span class="g">■<i>n</i></span><span class="m">the <b>chapter mark</b> — a mined block cited by its place within its book (n counts chapters, never a raw height); in a margin, the same glyph counts chapters of relative delay. <span class="why">The one place n is a raw height is the coinbase's BIP34 mark, above, where the miner wrote the number himself</span></span></div>
              <div class="glyph-row"><span class="g">‡<i>ₙ</i></span><span class="m">a <b>cited work</b> — a document outside this text, dated by the output this mark rides: its OpenTimestamps proof reduces the document's digest to this chapter's merkle root. The subscript is the work's number in the Citations register at the back. <span class="why">A reader's own kept proof wears the bare ‡, an addendum outside the edition's numbering. The one mark that points out of the book</span></span></div>
              <div class="glyph-row"><span class="g">□<i>n</i></span><span class="m">the <b>expected chapter mark</b> — a not-yet-mined block cited by the place it will take. <span class="why">From the queue, where the number holds only while the queue does, or from a height consensus has already fixed (the contents' Appendix II), where the number is as firm as any citation and only its block is missing (a bare □ in a transaction's margin is the no-locktime mark)</span></span></div>
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row"><span class="g">( )</span><span class="m"><b>run</b> what the lock hands back. <span class="why">Two kinds ride in it, and the difference is who wrote the program. A script the SPENDER revealed stands as its letter, still awaited: ( r ) ( w ) ( t ). A program CONSENSUS supplies stands written out, because it is already known — ( ⧉ ⌖ h²⁰ ≡ ∇ ) is the P2PKH template a v0 key-hash output is run as, ( Σ h³² = ) the commitment check before a witness script runs, ( ∇ ) the key-path signature check that runs no script at all. So a mark inside a ( ) is an operation the wire never spells, where every other mark on the line is a byte something wrote. Set P2SH beside P2WSH and the whole of segwit is one pair of parentheses: ⌖ h²⁰ = ( r ) checks on the wire, ⓪ h³² ( Σ h³² = ) ( w ) has the check supplied</span></span></div>
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row"><span class="g">∅</span><span class="m"><b>empty</b> — nothing is there. <span class="why">A coinbase's absent prevout, a segwit input's zero-byte script, an empty witness or one of its items, a field a flag zeroed</span></span></div>
