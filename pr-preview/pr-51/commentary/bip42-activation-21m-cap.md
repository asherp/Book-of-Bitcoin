...
For the transactions that follow, that means the coinbase of every later chapter pays its miner in fees alone. The §1 of a chapter after this height is still a transaction like any other — it still has the shape this book renders — but the amount it creates is zero, and everything it pays out was paid in by somebody else's transaction. From this height the chain issues nothing new; existing coins only circulate.
