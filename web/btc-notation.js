...
export const NOTATION_HTML = `
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row" data-marks="β* re:[0-9][⁰¹²³⁴⁵⁶⁷⁸⁹]"><span class="g">β<i>n</i> 2<sup><i>k</i></sup>·<i>p</i>…</span><span class="m">the <b>difficulty target</b> the miner restated in the coinbase, in the chapter head's own notation — a valid hash opens with n zero bits, and the target is exactly the product beside the mark. The preamble is the older custom: before BIP34 a coinbase opened with this pair instead of a height</span></div>
              <div class="glyph-row" data-marks="η*"><span class="g">η<sub><i>p</i></sub>·<sub><i>q</i></sub>…</span><span class="m"><b>extranonce</b> — search space beyond the header's η, the counter a miner rolls once the header's 32 bits are exhausted. It follows the opening mark in either era: behind ■ from BIP34 on, behind β before it, its value subscript in both. Factored like every other number the book states as a product, in the lowered register the glyph carries its values in: the primes drop with the value, the dot between them does not. Read as the number it is, like the header's own nonce — which also keeps it out of the margin below, where a counter's bytes are entropy and better than a third of them pass for printable text by chance</span></div>
