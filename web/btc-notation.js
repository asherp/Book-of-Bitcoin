...
export const NOTATION_HTML = `
          ...
          <div class="notation-group">
            ...
            <div class="glyph-grid">
              ...
              <div class="glyph-row" data-marks="zero:run"><span class="g">⓪<sup><i>n</i></sup></span><span class="m"><b><i>n</i> zero bytes</b> — in a coinbase, the space a pool's template laid out and nothing filled: room for the counter a miner rolls, or for a commitment this block is not carrying. In a witness or a script's own push it is whatever left the room empty there — a Taproot internal key chosen as a nothing-up-my-sleeve value, the padding inside an inscription's payload — and the bytes state only that they are zero, which is why the hover says so and no more. ⓪ is the zero opcode (0x00, the byte itself) and the superscript counts bytes, as every superscript in a script does. It is a reading of length, not of meaning: the count restores the bytes exactly, and the alternative is a paragraph of the wordlist's first word, which is what zero encodes to. The chapter head's ⓪<i>n</i> counts zero <i>bits</i> of a block hash — same glyph, and each register measures in the unit its own thing is measured in</span></div>
