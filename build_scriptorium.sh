...
# reasons: the engine is a Rust crate of its own (asherp/scriptorium),
# versioned and tested on its own, and the book builds against a pinned
# release of it rather than carrying a copy of the source.
#
...
# asherp/scriptorium triggers its publish workflow), set SCRIPTORIUM_VERSION
  ...
  if [ -n "$SCRIPTORIUM_VERSION" ]; then
    ...
    if [ ! -d "$SCRIPTORIUM_DIR" ]; then
      ...
      if ! curl -sSfL "https://static.crates.io/crates/scriptorium/scriptorium-$SCRIPTORIUM_VERSION.crate" -o "$crate"; then
        ...
        echo "       asherp/scriptorium triggers its publish workflow.) To build from a git" >&2
    ...
    if [ ! -d "$SCRIPTORIUM_DIR" ]; then
      ...
      if ! git clone --quiet --depth 1 --branch "$SCRIPTORIUM_REF" \
          https://github.com/asherp/scriptorium "$SCRIPTORIUM_DIR"; then
        echo "error: could not clone asherp/scriptorium at ref '$SCRIPTORIUM_REF'." >&2
